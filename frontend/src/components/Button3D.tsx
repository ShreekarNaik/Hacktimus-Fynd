  ...
  return (
    <motion.button
      ...
      className={clsx(
        ...
        variant === 'red' && "from-red-400 to-red-600 shadow-red-700",
        className
      )}
      style={{
           boxShadow: variant === 'green' ? '0 6px 0 #1e8449, 0 10px 10px rgba(0,0,0,0.2)' : 
                      variant === 'blue' ? '0 6px 0 #1c5980, 0 10px 10px rgba(0,0,0,0.2)' : 
                      variant === 'red' ? '0 6px 0 #991b1b, 0 10px 10px rgba(0,0,0,0.2)' : undefined
      }}
      {...props}
    >
      {label}
