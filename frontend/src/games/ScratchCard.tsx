...
const ScratchCard = () => {
  ...
  const checkReveal = () => {
    ...
    if (Math.random() < 0.1) {
      // 10% chance to check to save perf
  ...
  return (
    <Layout>
      <div className="flex flex-col items-center p-8">
        <h1 className="font-titan text-4xl text-blue-500 mb-8">
          LUCKY SCRATCH
        </h1>
        ...
        <div className="relative w-[300px] h-[150px] bg-white rounded-lg shadow-xl overflow-hidden mb-8 border-4 border-blue-200">
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-stripes-blue">
            <div className="text-center animate-pulse">
              <h3 className="font-black text-2xl text-gray-700">
                {prize?.label || "..."}
              </h3>
              {revealed && (
        ...
        {!revealed && (
          <p className="text-gray-400 font-bold text-sm animate-pulse">
            Scratch the silver area to reveal!
          </p>
        )}
        <button
          onClick={() => navigate("/")}
          className="mt-8 text-blue-300 font-bold underline"
        >
          BACK TO HOME
        </button>
      </div>
