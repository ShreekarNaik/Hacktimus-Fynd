            ...
            <div className="max-w-md mx-auto p-4 text-center">
                ...
                {!started && !finished && (
                    <div className="bg-white p-8 rounded-xl shadow-lg border-2 border-green-100">
                        <p className="mb-8 font-bold text-gray-500">Answer {QUESTIONS.length} questions correctly to win coins!</p>
                        <Button3D label="START QUIZ" onClick={handleStart} variant="green" />
                    </div>
                )}
                ...
                {started && !finished && (
                    <div className="animate-bounce-in">
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-md border-b-4 border-gray-200 mb-6">
                            <h2 className="text-xl font-bold text-gray-800">{QUESTIONS[currentQ].q}</h2>
                        </div>
                ...
                {finished && (
                     <div className="bg-white p-8 rounded-xl shadow-lg border-2 border-green-100 animate-bounce-in">
                         <h2 className="text-3xl font-black text-gray-800 mb-2">FINISHED!</h2>
                         <p className="text-lg text-gray-500 mb-6">You got <span className="text-green-500 font-bold">{score}/{QUESTIONS.length}</span> correct.</p>
                         
                         {lastResult && (
                             <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 mb-6">
                                 <div className="text-sm font-bold text-yellow-600">EARNED</div>
                                 <div className="text-3xl font-black text-yellow-500">{lastResult.coinsEarned} Coins</div>
                             </div>
                         )}
                         
                         <Button3D label="PLAY AGAIN" onClick={handleStart} />
                         <button onClick={() => navigate('/')} className="mt-4 text-gray-400 text-sm font-bold underline block w-full">EXIT</button>
                     </div>
                )}
            </div>
        </Layout>
