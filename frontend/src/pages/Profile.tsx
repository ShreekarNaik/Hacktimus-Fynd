...
const STORE_OPTIONS = ['Myntra', 'Ajio', 'Amazon', 'Flipkart', 'Nike', 'Adidas', 'Puma', 'H&M', 'Zara', 'Swiggy', 'Zomato'];
...
const Profile = () => {
    ...
    useEffect(() => {
        if (user) {
            fetchRewards();
            fetchPendingRewards();
            setEditName(user.userId || ''); // Assuming userId is the display name for now
    ...
    const handleClaim = async (gameName: string) => {
        ...
        try {
            ...
            setSelectedReward({ ...res.data.reward, gameName, company: 'Fynd Games' }); // Mock company if missing
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            await updateUserProfile(user!.userId, { userId: editName, preferredStores });
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await deleteUser(user!.userId, deleteOtp);
            ...
            <div className="p-4 max-w-lg mx-auto pb-24">
                ...
                <div className="bg-white rounded-[30px] p-6 shadow-xl border-4 border-purple-100 mb-8 relative">
                    ...
                    <button 
                        ...
                        className={`absolute top-4 right-4 w-10 h-10 rounded-full flex items-center justify-center text-xl shadow-sm border border-gray-100 transition-all ${isSettingsMode ? 'bg-orange-100 text-orange-500 rotate-90' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
                    >
                        ⚙️
                    </button>
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        {isSettingsMode ? (
                            <div className="mb-4 w-full animate-fade-in">
                                <label className="text-xs font-bold text-gray-400 uppercase mb-1 block text-center">Display Name</label>
                                <input 
                                    type="text" 
                                    value={editName}
                        ...
                        {isSettingsMode && (
                            <div className="mt-8 w-full animate-fade-in space-y-6">
                                <h3 className="font-titan text-gray-300 text-center text-lg border-b-2 border-dashed border-gray-200 pb-2">SETTINGS</h3>
                                ...
                                <div className="pt-2">
                                     <Button3D label="SAVE CHANGES" onClick={handleSaveProfile} variant="green" />
                                </div>
                        ...
                        {!isSettingsMode && preferredStores.length > 0 && (
                            <div className="mt-6 w-full">
                                <h3 className="text-xs font-bold text-gray-400 uppercase mb-2 text-center">My Preferred Coupon Companies</h3>
                                <div className="flex flex-wrap justify-center gap-2">
                                    {preferredStores.map(store => (
                                        <span key={store} className="bg-purple-50 text-purple-600 px-3 py-1 rounded-full text-xs font-bold border border-purple-100">
                                            {store}
                                        </span>
                                    ))}
                                </div>
                                 ...
                                 <div className="space-y-4">
                                     {pendingRewards.map((p, i) => (
                                         <div key={i} className="bg-gradient-to-r from-yellow-400 to-orange-400 p-4 rounded-2xl shadow-lg border-4 border-yellow-200 text-white relative overflow-hidden">
                                             ...
                                             <p className="text-xs font-bold w-2/3 mb-4">You are currently Rank #1! Claim your reward now.</p>
                                             <button 
                                ...
                                {rewards.map((reward, i) => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs font-bold text-gray-400">{reward.company || 'Fynd Games'}</div>
                ...
                <Modal isOpen={showDeleteModal}>
                    <div className="p-6 text-center w-full">
                        <h3 className="font-titan text-2xl text-red-500 mb-2">DELETE ACCOUNT</h3>
                        <p className="font-bold text-gray-500 text-xs mb-6">An OTP has been sent to your registered contact.</p>
                         {/* ... Existing OTP logic ... */}
                        <input 
                            type="text" 
                            value={deleteOtp}
                        ...
                        <div className="flex space-x-2">
                             <div className="flex-1">
                                 <Button3D label="CANCEL" onClick={() => setShowDeleteModal(false)} variant="blue" />
                             </div>
                             <div className="flex-1">
                                 <Button3D label="VERIFY" onClick={handleDeleteVerifyKey} variant="red" />
                             </div>
                        </div>
                    </div>
                </Modal>
                ...
                <Modal isOpen={showDeleteConfirmModal}>
                    <div className="p-6 text-center w-full">
                        ...
                        <h3 className="font-titan text-2xl text-gray-800 mb-2">ARE YOU SURE?</h3>
                        <p className="font-bold text-gray-500 text-sm mb-6">
                            This action is <span className="text-red-500">irreversible</span>. You will lose all your coins, wins, and claimed rewards.
                        </p>
