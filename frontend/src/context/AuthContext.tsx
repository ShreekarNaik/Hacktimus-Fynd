...
interface AuthContextType extends AuthState {
  ...
  register: (phoneNumber: string, username: string, otp: string) => Promise<void>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sendOtp = async (phoneNumber: string, isLogin: boolean = true) => {
    try {
      await client.post('/auth/send-otp', { phoneNumber, isLogin });
    } catch (e: any) {
      console.error('Failed to send OTP', e);
      // Propagate string error if possible
      const errorMessage = typeof e === 'string' ? e : (e.response?.data?.error || "Failed to send OTP");
      throw errorMessage;
    }
  };
  ...
  const loginWithOtp = async (phoneNumber: string, otp: string) => {
    try {
        const res = await client.post('/auth/login-otp', { phoneNumber, otp });
        const { token, user } = res.data;
        localStorage.setItem('token', token);
        localStorage.setItem('userId', user.userId);
        setState({ user, token, isAuthenticated: true, isLoading: false });
        ...
        const errorMessage = typeof e === 'string' ? e : (e.response?.data?.error || "Login Failed");
        throw errorMessage;
    }
  };

  const register = async (phoneNumber: string, username: string, otp: string) => {
    try {
        const res = await client.post('/auth/register', { phoneNumber, username, otp });
        const { token, user } = res.data;
  ...
  const updateUserProfile = async (userId: string, data: any) => {
      try {
          const res = await client.put('/user/profile', { userId, ...data });
          if (res.data.success) {
               // Optimistically update local state or re-fetch?
               // Let's just update local state with the data sent + response confirmation if needed
               setState(prev => ({
                  ...prev,
                  user: { ...prev.user, ...data } // merging updates
              }));
          }
      } catch (e: any) {
  ...
  return (
    <AuthContext.Provider value={{ ...state, login, logout, refreshProfile: fetchProfile, loginWithOtp, register, sendOtp, deleteUser, updateUserProfile }}>
      {children}
